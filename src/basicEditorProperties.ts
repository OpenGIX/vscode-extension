...
import { getLanguageService, SchemaRequestService, TextDocument } from 'vscode-json-languageservice';
...
export class BasicEditorProperties implements vscode.WebviewViewProvider {
    ...
    public static register(context: vscode.ExtensionContext): vscode.Disposable {
		...
		vscode.commands.registerCommand('panel.BasicEditorProperties.AddProperty', async () => {
            ...
            if (property !== "undefined") {
                vscode.window.showInformationMessage(`New property created: ${property}`);
            }
        });
        

        return vscode.window.registerWebviewViewProvider(this.viewType, new BasicEditorProperties(context));
    }
    ...
    public resolveWebviewView(panel: vscode.WebviewView, context: vscode.WebviewViewResolveContext<unknown>, token: vscode.CancellationToken): void | Thenable<void> {
        ...
        vscode.window.onDidChangeTextEditorSelection((event) => {
            ...
            let selection = editor.document.lineAt(editor.selection.active.line).text.trim().replace(/,$/, "");
            ...
            const textDocument = TextDocument.create('https://geojson.org/schema/GeoJSON.json', 'json', 1, selection);
            ...
            jsonLanguageService.configure({ allowComments: false, schemas: [{ fileMatch: ["*.json"], uri: jsonSchemaUri }] });
    ...
    private getEditableDataGridMarkup(webview: vscode.Webview) {
        ...
        const scriptUri = webview.asWebviewUri(vscode.Uri.joinPath(this.context.extensionUri, 'dist', 'editableTableGrid.js'));	
        const styleSheetUri = webview.asWebviewUri(vscode.Uri.joinPath(this.context.extensionUri, 'media', 'style.css'));	
