		...
		const items = <vscode.QuickPickItem[]>[
			{
				...
				detail: "Every Feature object is a GeoJSON object no matter where it occurs in a GeoJSON text."
			},
			{
				label: "Feature Collection",
				description: "A FeatureCollection is a collection of GeoJSON Feature objects.",
 ...
 * 
 */
class DataProvider implements vscode.TreeDataProvider<Node>, vscode.TreeDragAndDropController<Node> {
	...
	constructor() {
		...
		vscode.commands.registerCommand('layersExplorerPanel.toggleVisibility', item => this.toggleVisbility(item));
		
		const document = JSON.parse(<string>vscode.window.activeTextEditor?.document.getText());
	...
	public async handleDrop(target: Node | undefined, sources: vscode.DataTransfer, token: vscode.CancellationToken): Promise<void> {
		const transferItem = sources.get('application/vnd.code.tree.canvasLayersExplorer');
		if (!transferItem) {
			return;
		}
	...
	public async handleDrag(source: Node[], treeDataTransfer: vscode.DataTransfer, token: vscode.CancellationToken): Promise<void> {
		treeDataTransfer.set('application/vnd.code.tree.canvasLayersExplorer', new vscode.DataTransferItem(source));
	}
	...
	_getTreeItem(key: string): vscode.TreeItem {
		...
		return {
			...
			tooltip,
			command: { command: 'layersExplorerPanel.onClick', title: 'Test', arguments: treeElement},
